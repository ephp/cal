<?php
namespace Ephp\CalendarBundle\Vendor\ICalCreator;
/*********************************************************************************/
/**
 * iCalcreator v2.16.6
...
class ICalendarFactory {
    ...
    public static $ICALCREATOR_VERSION = 'iCalcreator 2.16.6';
    /*********************************************************************************/
     ...
     * @return mixed
     */
    public static function iCal2vCard( $email, $version='2.1', $directory=FALSE, $ext='vcf' ) {
      ...
      $vCard .= 'PRODID:-//kigkonsult.se '.self::$ICALCREATOR_VERSION."//$nl";
      $vCard .= $N;
      $vCard .= $FN;
      $vCard .= $EMAIL;
     ...
     * @return mixed
     */
    public static function iCal2vCards( & $calendar, $version='2.1', $directory=FALSE, $ext='vcf' ) {
      $hits   = array();
    ...
    public static function iCal2XML( & $calendar ) {
      ...
      $xmlstr    .= '<!-- created utilizing kigkonsult.se '.self::$ICALCREATOR_VERSION.' iCal2XMl (rfc6321) -->';
      $xmlstr    .= '</icalendar>';
      ...
      foreach( $calProps as $calProp ) {
        if( FALSE !== ( $content = $calendar->getProperty( $calProp )))
          self::_addXMLchild( $properties, $calProp, 'text', $content );
      }
      while( FALSE !== ( $content = $calendar->getProperty( FALSE, FALSE, TRUE )))
        self::_addXMLchild( $properties, $content[0], 'unknown', $content[1]['value'], $content[1]['params'] );
            ...
            switch( strtolower( $prop )) {
              case 'attach':          // may occur multiple times, below
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  ...
                  self::_addXMLchild( $properties, $prop, $type, $content['value'], $content['params'] );
                }
                break;
              case 'attendee':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  ...
                  self::_addXMLchild( $properties, $prop, 'cal-address', $content['value'], $content['params'] );
                }
                break;
              case 'exdate':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
              ...
              case 'freebusy':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  ...
                  self::_addXMLchild( $properties, $prop, 'period', $content['value'], $content['params'] );
                }
                break;
              case 'request-status':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  ...
                  self::_addXMLchild( $properties, $prop, 'rstatus', $content['value'], $content['params'] );
                }
                break;
              case 'rdate':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
              ...
              case 'resources':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  ...
                  self::_addXMLchild( $properties, $prop, 'text', $content['value'], $content['params'] );
                }
                break;
              case 'x-prop':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
              ...
              case 'duration':
                if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  ...
                  self::_addXMLchild( $properties, $prop, 'duration', $content['value'], $content['params'] );
                }
                break;
              case 'rrule':
                while( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
                  self::_addXMLchild( $properties, $prop, 'recur', $content['value'], $content['params'] );
                break;
              case 'class':
              case 'location':
              case 'status':
              ...
              case 'geo':
                if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
                  self::_addXMLchild( $properties, $prop, 'geo', $content['value'], $content['params'] );
                break;
              case 'organizer':
                if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE ))) {
                  if( isset( $content['params']['CN'] ) && !isset( $content['params']['LANGUAGE'] )) {
              ...
              case 'sequence':
                if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
                  self::_addXMLchild( $properties, $prop, 'integer', $content['value'], $content['params'] );
                break;
              case 'tzurl':
              case 'url':
                if( FALSE !== ( $content = $component->getProperty( $prop, FALSE, TRUE )))
                  self::_addXMLchild( $properties, $prop, 'uri', $content['value'], $content['params'] );
                break;
            } // end switch( $prop )
                ...
                switch( strtolower( $prop )) {
                  ...
                  case 'dtstart':
                    if( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE ))) {
                      ...
                      self::_addXMLchild( $properties, $prop, 'date-time', $content['value'], $content['params'] );
                    }
                    break;
                  case 'duration':
                    if( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE )))
                  ...
                  case 'tzoffsetfrom':
                    if( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE )))
                      self::_addXMLchild( $properties, $prop, 'utc-offset', $content['value'], $content['params'] );
                    break;
                  case 'rrule':
                    while( FALSE !== ( $content = $subcomp->getProperty( $prop, FALSE, TRUE )))
                      self::_addXMLchild( $properties, $prop, 'recur', $content['value'], $content['params'] );
    ...
    public static function & XMLstr2iCal( $xmlstr, $iCalcfg=array()) {
      ...
      return self::xml2iCal( $xml, $iCalcfg );
    }
    /**
     ...
     * @return mixed  iCalcreator instance or FALSE on error
     */
    public static function & XML2iCal( $xmlobj, $iCalcfg=array()) {
          ...
          if( 'components' == $calPart->getName()) {
            foreach( $calPart->children() as $component ) { // single components
              if( 0 < $component->count())
                self::_getXMLComponents( $iCal, $component );
            }
          }
             ...
             $propName = $calProp->getName();
              ...
              foreach( $calProp->children() as $calPropElem ) { // single calendar property
                if( 'parameters' == $calPropElem->getName())
                  $params = self::_getXMLParams( $calPropElem );
                else
     ...
     */
    public static function _getXMLParams( & $parameters ) {
     ...
     * @return void
     */
    public static function _getXMLComponents( & $iCal, & $component ) {
     ...
     * @return void
     */
    public static function _getXMLProperties( & $iCal, & $property ) {
      ...
      foreach( $property->children() as $propPart ) { // calendar property parameters (opt) and value(-s)
        ...
        if( 'parameters' == $valueType) {
          $params = self::_getXMLParams( $propPart );
          continue;
        }
        switch( $valueType ) {
          case 'binary':
     ...
     *
     */
    public static function getTzOffsetForDate($timezonesarray, $tzid, $timestamp) {
